...
namespace runnerDotNet
{
	...
	public static class MVCFunctions
	{
		...
		private static readonly DateTime PHP_NULL_TIME = new DateTime(1970, 1, 1, 0, 0, 0, 0);
        ...
        public static XVar bin2hex(XVar binVal)
        {
			...
			if(binVal.IsByteArray())
			{
				StringBuilder result = new StringBuilder();
				byte[] byteArray = binVal.Value as byte[];
			...
			{
				StringBuilder result = new StringBuilder();
				string binStr = binVal.ToString();
				for (int i = 0; i < binStr.Length; i++)
				{
					result.Append(Encoding.Default.GetBytes(binStr.Substring(i, 1))[0].ToString("X"));
				}
				return result.ToString();
			}
        }
...
//			HttpContext.Current.Response.Write(GlobalVars.Buffer.ToString());
        ...
        public static XVar set_error_handler(XVar handler)
        {
			GlobalVars.strErrorHandler = handler;
			return null;
        }

		public static XVar restore_error_handler()
		{
			GlobalVars.strErrorHandler = null;
			return null;
		}

		public static XVar HandleError()
		{
			if (GlobalVars.strErrorHandler != null)
				return true;
			else return false;
		}
		...
		{
            return (DateTime.Now - PHP_NULL_TIME).TotalSeconds;
		}
        ...
        public static XVar mktime(XVar hour, XVar minute, XVar second, XVar month, XVar day, XVar year)
		{
			...
			dayNum = dayNum > maxDays ? maxDays : (dayNum < 1 ? 0 : dayNum);
        ...
        {
            XVar result = templateName.ToString() + ".cshtml";
	        if(table != "")
            {
        ...
        public static void HeaderRedirect(XVar header)
        {
			...
			if (url != "")
				throw new RunnerRedirectException(url[0] == '/' || Uri.IsWellFormedUriString(header, UriKind.Absolute) ? url : GetTableLink(header).ToString());
        }

        public static void HeaderRedirect(XVar table, XVar action = null, XVar getParams = null)
        {
            throw new RunnerRedirectException(GetTableLink(table, action, getParams));
        }
		...
		public static XVar runner_mail(XVar var_params)
		{
            ...
            try
            {
				...
				if( "localhost" != ""  ||  var_params.KeyExists("host") )
				{
				...
				if( "25" != ""  ||  var_params.KeyExists("port") )
				{
					if( var_params.KeyExists("port") )
						...
						message.Configuration.Fields["http://schemas.microsoft.com/cdo/configuration/smtpserverport"].Value = int.Parse("25");
				}
				...
				if( "" != ""  ||  var_params.KeyExists("username") )
				{
					// use basic clear-text authentication
					message.Configuration.Fields["http://schemas.microsoft.com/cdo/configuration/smtpauthenticate"].Value = 1;

					message.Configuration.Fields["http://schemas.microsoft.com/cdo/configuration/sendusername"].Value = var_params.KeyExists("username") ? var_params["username"].ToString() : "";
					message.Configuration.Fields["http://schemas.microsoft.com/cdo/configuration/sendpassword"].Value = var_params.KeyExists("password") ? var_params["password"].ToString() : "";
				}
			...
			return new XVar("mailed", isMailed, "errors", smtpResponse, "message", smtpResponse);
		}

		// runner_mail that uses System.Net.Mail #11608
		public static XVar runner_mail_old(XVar var_params)
		{
			bool isMailed = false;
            string smtpResponse = "";
            try
            {
				...
				if( "" != ""  ||  var_params.KeyExists("username"))
				{
					...
					smtp.Credentials = new NetworkCredential(
						var_params.KeyExists("username") ? var_params["username"].ToString() : "",
						var_params.KeyExists("password") ? var_params["password"].ToString() : ""
						);
				}
				else
				{
        ...
        public static XVar myfile_get_contents(XVar file, XVar mode = null)
		{
            ...
            else return File.ReadAllText(file, Encoding.GetEncoding((int) GlobalVars.cCodepage));
		}

		public static XVar myfile_get_contents(HttpPostedFile file, XVar mode = null)
		{
			if (mode as object == null)
				mode = "rb";
		...
		{
            return new WebClient().DownloadString(url.ToString());
		}
		...
		{
			string file = _param_file.ToString();
		...
		{
			return HttpContext.Current.Request.Files[name.ToString()].FileName;
		}
		...
		public static void runner_save_file(XVar filename, XVar contents)
		{
			...
			else File.WriteAllText(filename.ToString(), contents.ToString());
		}
		...
		public static XVar getDashMapCustomIcon(XVar dashName, XVar dashElementName, XVar data)
		{
			return CustomExpressionProvider.Instance.getDashMapCustomIcon(dashName, dashElementName, data);
		}
		...
		{
			return CustomExpressionProvider.Instance.GetCustomExpression(file, data, field, ptype, table);
		}
		...
		public static void echoBinary(XVar output, XVar _param_bufferSize = null)
		{
			...
			int totalChars = (output.Value is byte[]) ? (output.Value as byte[]).Length : strOutput.Length;

			for(int i = 0; i <= totalChars; i += bufferSize)
			{
				if (output.Value is byte[])
				{
				...
				else HttpContext.Current.Response.Write(strOutput.Substring(i, bufferSize));
			}
		}
...
//          MVCFunctions.Echo(bufferBkf + rendered);
        ...
        {
			XVar height_old, width_old;

            if (value == null || value.Value == null || (value.Value as byte[]).Length == 0)
			{
				return value;
			}
            MemoryStream ms = new MemoryStream(value.Value as byte[]);
            Image originalImage = Image.FromStream(ms);
            if (originalImage == null)
			{
				return value;
			}
            width_old = originalImage.Size.Width;
            height_old = originalImage.Size.Height;
			if(size < width_old  ||  size < height_old)
			{
                ...
                ImageFormat imageFormat = GetImageFormatByExt(fileExt.ToLower());

                ms = new MemoryStream();
                resizedImage.Save(ms, imageFormat);

                return new XVar(ms.ToArray());
			}
            return value;
        }
		...
		{
            bool result = true;
            Image originalImage = Image.FromStream((uploadedFile["file"].Value as HttpPostedFile).InputStream);
            ...
            {
                Bitmap resizedImage = new Bitmap((int)destWidth, (int)destHeight, PixelFormat.Format32bppPArgb);
                ResizeImage(sourceWidth, sourceHeight, originalImage, destWidth, destHeight, resizedImage);

                ImageFormat imageFormat = GetImageFormatByExt(file_path.Substring(file_path.LastIndexOf(".") + 1).ToLower().ToString());

                resizedImage.Save(new_file_path, imageFormat);
        ...
        {
            string result = new FileInfo(path).Name;
        ...
        public static XVar pathinfo(XVar path)
        {
            ...
            XVar result = new XVar("basename", fileInfo.Name);
        ...
        public static string RenderSectionToStringFromString<T>(string templateString, string sectionName, T model)
        {
            if(sectionName != "")
            {
                int startPosition = templateString.IndexOf("@foreach (string _" + sectionName);
                if (startPosition < 0)
                    return "";
                int endPosition = 0;
                int bracesCount = 0;
		...
		public static void SetCookie(XVar name, XVar value, XVar expire = null, XVar path = null, XVar domain = null, XVar secure = null, XVar httponly = null)
        {
...
//			HttpContext.Current.Response.Cookies.Remove(name);
		...
		public static XVar findMatches( XVar pattern, XVar str )
		{
			XVar matches = XVar.Array();
			str.PregFindMatches(pattern, matches);
			return matches;
		}
        ...
        public static XVar md5(XVar plainText)
        {
		...
		public static XVar runner_strpos(dynamic haystack, dynamic needle, dynamic offset = null)
		{
			...
			if (offset >= 0)
				return str.IndexOf(needle.ToString(), offset);
			else
				return str.IndexOf(needle.ToString(), str.Length + offset, - offset);
		}
		...
		public static XVar runner_strrpos(dynamic haystack, dynamic needle, dynamic offset = null)
		{
			...
			string str = haystack.ToString();
			if (offset >= 0)
				return str.LastIndexOf(needle.ToString(), str.Length - 1, str.Length - offset);
			else
				return str.LastIndexOf(needle.ToString(), str.Length - 1, - offset);
		}

		public static XVar runner_substr(dynamic str, dynamic start, dynamic length)
		{
		...
		public static XVar cutBOM(dynamic _param_line)
		{
		...
		public static XVar runner_save_textfile(dynamic _param_fileName, dynamic _param_txtData)
		{
			...
			StreamWriter outfile = new StreamWriter((string)_param_fileName);
		...
		public static bool passwordVerify(XVar pass, XVar hash)
		{
			...
			return BCrypt.Net.BCrypt.Verify(pass.ToString(), strHash);
		}


        public delegate XVar dashSnippetDelegate( ref XVar header );

		public static XVar callDashboardSnippet(dynamic _param_name, object _param_eventsObject)
		{
			dynamic snippetData, header = new XVar("");
			MVCFunctions.ob_start();


            var arguments = new object[] { header };
			MethodInfo method = _param_eventsObject.GetType().GetMethods().Where(m => m.Name == _param_name.ToString()).First();
			method.Invoke(_param_eventsObject, arguments );
			header = arguments[0];
		...
		public static XVar getArrayWithoutObjects(dynamic _param_arr)
		{
			throw new NotImplementedException();
		}
		...
		{
			return CommonFunctions.common_runner_sms(_param_number, _param_message, _param_parameters);
		}
